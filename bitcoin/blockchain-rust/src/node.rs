...
impl Nodes {
    ...
    pub fn add_node(&self, addr: String) {
        ...
        if let None = inner.iter().position(|item| item.get_addr().eq(addr.as_str())) {
    ...
    pub fn evict_node(&self, addr: &str) {
        let mut inner = self.inner.write().unwrap();
        if let Some(idx) = inner.iter().position(|item| item.get_addr().eq(addr)) {
            inner.remove(idx);
        }
    }
    ...
    pub fn node_is_known(&self, addr: &str) -> bool {
        ...
        if let Some(_) = inner.iter().position(|item| item.get_addr().eq(addr)) {
            return true;
        }
